...
def evaluate_model_performance(model, val_images_path, annotations_path):
    ...
    try:
        results = model.val(data=dataset_config, verbose=False, save=False, plots=False)
        ...
        if model_info["model_path"] and os.path.exists(model_info["model_path"]):
            model_info["model_size_mb"] = os.path.getsize(model_info["model_path"]) / (
                1024 * 1024
            )
...
if __name__ == "__main__":
    ...
    preformance_pretrained = evaluate_model_performance(
        model_pretrained, coco_val_path, coco_ann_path
    )
