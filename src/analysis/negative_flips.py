...
def find_matching_detections(
    detections: List[Dict], gt_objects: List[Dict], iou_threshold: float = 0.5
) -> Dict:
    """
...
def analyze_negative_flips(
    results_v1: Dict, results_v2: Dict, coco: COCO, iou_threshold: float = 0.5
) -> Dict:
    """
    ...
    CNF_rate_standard = (
        CNF_total / N_loc if N_loc > 0 else 0
    )  # Standard: denominator = N_loc
    CNF_rate_common_denom = (
        CNF_total / N_total if N_total > 0 else 0
